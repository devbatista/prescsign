class AuditLogPolicy < ApplicationPolicy
  ...
  class Scope < Scope
    def resolve
      ...
      owner_document_ids = Document.where(organization_id: current_organization_id, doctor_id: user.id).select(:id)
      owner_patient_ids = Patient.where(organization_id: current_organization_id, doctor_id: user.id).select(:id)
      ...
      scope_by_actor = tenant_scope.where(actor_type: "Doctor", actor_id: user.id)
