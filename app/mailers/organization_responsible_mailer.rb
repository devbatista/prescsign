...
class OrganizationResponsibleMailer < ApplicationMailer
  def signup_invitation
    ...
    @registration_url = "#{frontend_base_url}/auth/register?invitation_token=#{CGI.escape(@invitation_token)}"
  ...
  def frontend_base_url
    protocol = Rails.application.config.x.frontend_protocol
    host = Rails.application.config.x.frontend_host
    port = Rails.application.config.x.frontend_port.to_i
